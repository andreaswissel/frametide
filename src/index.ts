...
export class FigmaMcpServer {
  ...
  private setupHandlers() {
        ...
        tools: [
          {
            name: "get-component",
            description:
              "Extract detailed information about a specific Figma component",
            inputSchema: {
              type: "object",
              properties: {
                fileId: {
          ...
          },
          {
            ...
            description:
              "Get comprehensive component specification for any framework",
            inputSchema: {
              type: "object",
              properties: {
                fileId: {
          ...
          },
          {
            ...
            description:
              "Check if Figma components have been modified since last sync",
            inputSchema: {
              type: "object",
              properties: {
                fileId: {
          ...
          },
          {
            ...
            description:
              "Set the current working Figma file from a URL for the session",
            inputSchema: {
              type: "object",
              properties: {
                url: {
                  type: "string",
                  description:
                    "Figma file URL (e.g., https://www.figma.com/design/FILE_ID/File-Name)",
                },
              },
              required: ["url"],
            },
          },
          {
            ...
            description:
              "Get information about the current working file and implementation status",
            inputSchema: {
              type: "object",
              properties: {},
            },
          },
          {
            ...
            description:
              "Get all components organized by implementation status",
            inputSchema: {
              type: "object",
              properties: {
                filter: {
          ...
          },
          {
            ...
            description:
              "Get comprehensive implementation details for a specific component",
            inputSchema: {
              type: "object",
              properties: {
                componentId: {
    ...
    this.server.setRequestHandler(CallToolRequestSchema, async (request) => {
      ...
      try {
        ...
        // Security validation
        const sanitizedArgs = SecurityValidator.validateAndSanitizeArgs(
          args || {}
        );
        ...
        switch (name) {
          ...
          case "get-implementation-queue":
            result = await this.handleGetImplementationQueue(
              sanitizedArgs,
              clientId
            );
            break;
          ...
          case "get-component-for-implementation":
            result = await this.handleGetComponentForImplementation(
              sanitizedArgs,
              clientId
            );
            break;
          ...
          case "update-component-status":
            result = await this.handleUpdateComponentStatus(
              sanitizedArgs,
              clientId
            );
            break;
  ...
  private async handleListComponents(args: any) {
    ...
    const components = await this.componentExtractor.listComponents(
      fileId,
      filter
    );
  ...
  private async handleGetComponentSpecification(args: any) {
    ...
    const specification =
      await this.componentExtractor.extractComponentSpecification({
        fileId,
        componentId,
        includeAccessibility,
        includeInteractions,
  ...
  private async handleSetWorkingFile(args: any, clientId: string) {
    ...
    try {
      ...
      const workingFile = this.sessionManager.setWorkingFile(
        clientId,
        parsedUrl,
        file.name
      );
            ...
            text: JSON.stringify(
              {
                ...
                message:
                  "No working file set. Use set-working-file to specify a Figma file URL.",
              },
              null,
              2
            ),
  ...
  private async handleGetImplementationQueue(args: any, clientId: string) {
    ...
    const { filter } = args;
    const componentsResult = await this.componentExtractor.listComponents(
      workingFile.fileId,
      filter
    );
    const queue = this.sessionManager.getImplementationQueue(
      clientId,
          ...
          text: JSON.stringify(
            {
              ...
              queue: {
                total: queue.total,
                pending: queue.pending.map((c) => ({
                  id: c.id,
                  name: c.name,
                  type: c.type,
                })),
                inProgress: queue.inProgress.map((c) => ({
                  id: c.id,
                  name: c.name,
                  type: c.type,
                })),
                implemented: queue.implemented.map((c) => ({
                  id: c.id,
                  name: c.name,
                  type: c.type,
                })),
                needsUpdate: queue.needsUpdate.map((c) => ({
                  id: c.id,
                  name: c.name,
                  type: c.type,
                })),
  ...
  private async handleGetComponentForImplementation(
    args: any,
    clientId: string
  ) {
    const workingFile = this.sessionManager.getWorkingFile(clientId);
    ...
    const status = this.sessionManager.getComponentStatus(
      clientId,
      componentId
    );
  ...
  private async handleUpdateComponentStatus(args: any, clientId: string) {
    ...
    const updatedStatus = this.sessionManager.getComponentStatus(
      clientId,
      componentId
    );
    ...
    switch (framework) {
      case "react":
        return {
          ...
          exampleUsage: `<${this.toPascalCase(
            component.name
          )} ${this.generateReactProps(component)} />`,
      ...
      case "angular":
        return {
          ...
          inputs: component.componentInterface.props.map(
            (prop: any) => `@Input() ${prop.name}: ${prop.type};`
          ),
          outputs: component.componentInterface.events.map(
            (event: any) =>
              `@Output() ${event.name} = new EventEmitter<${event.type}>();`
          ),
        };
      ...
      case "vue":
        return {
          ...
          emits: component.componentInterface.events.map(
      ...
      case "svelte":
        return {
          componentName: this.toPascalCase(component.name),
          props: component.componentInterface.props.map(
            (prop: any) =>
              `export let ${prop.name}${
                prop.required ? "" : " = " + JSON.stringify(prop.default)
              }: ${prop.type};`
          ),
          events: component.componentInterface.events.map(
            (event: any) => event.name
          ),
        };
      ...
      default:
        return {
          ...
          notes:
            "Generic implementation hints - adapt to your framework of choice",
        };
    }
  }
  ...
  private generateReactProps(component: any): string {
    return component.componentInterface.props
      ...
      .map(
        (prop: any) =>
          `${prop.name}={${JSON.stringify(prop.default || "value")}}`
      )
      .join(" ");
  }

  private generateReactStateHints(component: any): any {
    const hasInteractiveState = component.componentInterface.props.some(
      (prop: any) =>
        ["checked", "selected", "expanded", "active"].includes(
    ...
    return {
      ...
      suggestions: hasInteractiveState
        ? ["useState for interactive state", "useCallback for event handlers"]
        : [],
    };
  }
